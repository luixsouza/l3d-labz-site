...
| 260616-m6a | **Varredura WhatsApp→Instagram** (a marca não tem WhatsApp; contato é `@l3d_labz`): 9 templates trocados (home "Precisa de ajuda?", about 4×, footer, privacidade/termos, calculadora — removido o número FALSO `wa.me/5511999999999`) usando `{{ SITE.instagram }}` + `#i-instagram`; 3 importadores (`importar_makerworld`/`importar_copa`/`seed_makerworld`) geram descrição com Instagram; comentário do mapper. Comando novo `corrigir_contato_descricoes` (`--dry-run`/`--limite`, idempotente) reescreve "WhatsApp"→"Instagram @l3d_labz" nas descrições existentes — verificado local (24 alterados). `grep -riE "whatsapp|wa.me" apps/` agora VAZIO. **De quebra corrigido bug pré-existente: /privacidade/ e /termos/ davam 500** (`{% block title %}` duplicado dentro de `{% if %}` desde 1c969dc; unificado — ad97a92). VERIFICADO no browser. ✅ **`corrigir_contato_descricoes` JÁ RODADO em prod (88 descrições corrigidas)** via novo workflow `manage.yml`. | 2026-06-16 | b3dc752, 7ee3c96, cc78571, ad97a92 |
| ops | Novo workflow **`.github/workflows/manage.yml`** (dispatch manual) roda qualquer `manage.py <cmd>` no container `web` de prod via Tailscale+SSH (reusa secrets do deploy). Uso: `gh workflow run manage.yml -f command="<cmd> --dry-run"`. Usado p/ `corrigir_contato_descricoes`; serve também p/ `extrair_specs_3mf` quando quiserem acender os filtros de filamento/cores. | 2026-06-16 | 61f2ba6 |
| 260616-lol | **Redesign "Vibrante Maker"** (sketch 002-B escolhido pelo dono) aplicado ao site todo via **apêndice no `theme.css`** (rebrand por tokens, não reescrita das 2644 linhas) + `home.html`: tokens `--ink`/`--offset-shadow`; bordas duras + sombra-offset nos CTAs (`.btn-primary` hover translada, active recua) e hover de cards; `.product-thumb` em bloco pastel (`color-mix` com accent da categoria); badges/pills/chips com borda dura; hero `.geek-hero--clean` com degradê verde→azul + textura de pontos; **CTA da home WhatsApp→Instagram** (`.ig-cta` degradê IG → `SITE.instagram`). **Cards de altura IGUAL por fileira** (b7fcb1d: wrapper grid→flex + `line-clamp:2` + botão `margin-top:auto`; corrige o duplo `margin-top:auto`). Light primário, dark adaptado. **CORREÇÃO (dcf2b19):** a 1ª tentativa (apêndice) ficou "meia boca" porque PERDIA a guerra de especificidade pras regras `[data-theme="light"]`/`!important` existentes. Reescrito como **bloco autoritativo** (`!important` onde o código já usa, selettores `[data-theme]`-prefixados) que de fato reproduz o mockup: hero bordado 3px + degradê + textura de pontos + cartão 3D branco com sombra-offset + flag NOVO, headline "Bota pra imprimir", cards/botões/chips/header/faixas com borda dura + offset. Comparado lado-a-lado com o mockup no browser (light+dark). **VERIFICADO no browser** (Playwright, light+dark+mobile): home/catálogo/modelos-3d com estilo B e cards alinhados; 3D do detalhe intacto (modelo carrega), FAB/orçamento público/calculadora/carrinho OK. ⚠️ descrições de produto ainda citam "WhatsApp" (texto do importador — limpeza futura no server). | 2026-06-16 | f299d3f, 13c09fb, b7fcb1d |
| 260616-gj3 | Loja mais rica (Parte B): **`/catalogo/modelos-3d/` virou catálogo completo** — paginação + filtros + ordenação, reusando `product_list.html` parametrizado via `only_3d` (template `models_3d.html` deletado). `ProductQuery.search` ganhou `only_3d`, filtro `color` (1/2/3/"4+"), filtro `filament` por faixa (até 50g/50–150g/150g+; `filament_grams=0` nunca casa), e sorts `filament_asc`/`filament_desc`/`colors_desc` (0g vai pro fim via anotação `Case/When`). **Paginação agora preserva TODOS os filtros** via tag nativa `{% querystring %}` do Django 5.2 (corrige bug que perdia q/material/preço). `PAGE_SIZE` 12→24. Grid densificado (`minmax(210px,1fr)`, 2 col mobile) + stagger do reveal capado (`min(--i*35ms,420ms)`) → acaba o "vazão branco". Chip `.spec-chip` no card ("120 g · 3 cores"). **VERIFICADO no browser** (seed local): filtros cor/filamento batem com ORM, sort com 0 no fim, paginação preserva filtros, grid denso 24 cards, plural pt-br corrigido (8cc4d1c: "cores" não "cors"). **Decisão do dono: GLB fora** (não gerar mais GLBs; /modelos-3d/ fica com os 5 que já têm). Filtros de filamento/cores **ocultados até a extração rodar** via flag `specs_ready` (`ProductQuery.specs_available`, cache curto, auto-detecta `filament_grams>0`) — em prod ficam escondidos, reaparecem sozinhos pós-`extrair_specs_3mf` sem novo deploy (de7bd17). | 2026-06-16 | 3119899, 3b78b37, 603d9f2, 8cc4d1c, de7bd17 |
...
| 260617-mrl | **Calculadora de Custos no layout do mockup** (estilo 3dprime) + **modelo de cálculo ENXUTO** + navbar/footer no skin maker. `/calculadora/` reescrita em **coluna única** com 3 cards (Filamento / Energia e Tempo / Opcionais): chips de potência (Ender3 ~120W / Bambu A1 ~200W / X1C ~350W), chips de bandeira (verde/amarela/vermelha → preenchem `valor_kwh`), chips de margem (30/50/80/100), botões "Puxar preço do site"/"Restaurar preço automático" (preço de ref. L3D via `FILAMENTOS[...].preco_kg_default`), "Calcular Custo"/"Limpar" e breakdown full-width abaixo. **Fórmula enxuta** (decisão do dono): `material+energia+mão de obra+custos_fixos` → margem; REMOVIDOS depreciação/vida útil e taxa de falha; energia vira `valor_kwh` único. `PricingService`/`CalcForm.to_calc_data`/`presets.py` (`CONSUMO_CHIPS`, `BANDEIRA_KWH`)/`calculator.js` reescritos e espelhados; staff orçamento + PDF preservados (só dados públicos). **Navbar/footer** sincronizados ao skin maker que o CSS já esperava: `.brand-badge`+`logo.png`, "Labz" verde, `.footer-brand-block`/`.mascot-name`/`.mascot-sub`. Ícones de seção/botão remapeados pro sprite existente. **VERIFICADO por screenshot headless** (Playwright, claro+escuro): bate com `image.png`; ex. do mockup (130g, R$120/kg, 200W, 6h, kWh 1,00, mão de obra 20, margem 100%) = **R$ 73,60** ✓. | 2026-06-17 | e6a9e90, f9555b0, 42a55e4, 808e50a, 63ca075, 0229677 |
| 260626-o3a | **Zerar catálogo + admin de cadastro manual** (decisão do dono: apagar tudo e cadastrar à mão, pois as fotos do import eram lixão/loremflickr). Comando `limpar_catalogo` (dry-run por padrão; `--confirmar` → backup JSON timestamped via `dumpdata`→StringIO→UTF-8 em `backups/`, depois `Product.objects.all().delete()` — galeria cai por CASCADE, pedidos intactos via `OrderItem.product=SET_NULL`, categorias nunca tocadas por PROTECT; imprime contagens antes/depois + comando de restore `loaddata`). `ProductAdmin` turbinado: thumbnail 48px na listagem, preview 200px no form, preview por foto no `ProductImageInline`, fieldsets ordenados (básico→imagem→preço/estoque→impressão→destaque) com "Modelos 3D" e "Automação/scraper" colapsados; `CategoryAdmin` intocado. Verificado LOCAL (seed SQLite): `check` OK, dry-run não apaga, `--confirmar` → backup 18.9KB + `Product.count()==0` + 7 categorias, `loaddata` restaura. | 2026-06-26 | a563d02, 796d86c, c06dd7c, ee93502 |

## Session Continuity

Last session: 2026-06-26 (quick 260626-o3a — zerar catálogo + admin de cadastro manual; verificado local; deploy + wipe em prod + verificação no browser em andamento). Antes: 2026-06-17 (quick 260617-mrl — Calculadora de Custos no layout do mockup + modelo enxuto + navbar/footer no skin maker; verificado por screenshot claro/escuro). Antes: 2026-06-15 (quick 260615-sf7 — PDF de orçamento PREMIUM; antes: 260615-s0l redesign clean do PDF; 260615-nzr campos vazios na calculadora; pacote Shopee dos 669 modelos MakerWorld; superusuário l3dlabzz@gmail.com em prod)

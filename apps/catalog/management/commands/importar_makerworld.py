"""Importa pastas scrapadas do MakerWorld como produtos sob demanda.
...
Foto principal = sorted(fotos/*)[0]; GIF → converte primeiro frame p/ JPEG quadrado.
Se houver modelo.3mf: gera dimensões reais (mm→cm) e GLB p/ o visualizador 3D.
Se não houver 3mf: produto foto-only (sem dimensions, sem model_3d).

O STL original NUNCA é publicado — o GLB decimado serve só de preview no viewer.
...
Uso:
    ...
    python manage.py importar_makerworld --base /caminho --limite 5 --sem3d
...
# ---------------------------------------------------------------------------
# Processamento de foto principal (foto real, padronizada p/ quadrado)
# ---------------------------------------------------------------------------
...
class Command(BaseCommand):
    help = "Importa pastas scrapadas do MakerWorld como produtos sob demanda."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--sem3d", action="store_true",
            help="Cria produto foto-only, sem gerar GLB (p/ malhas que estouram a RAM).",
        )
        parser.add_argument(
            "--categoria", default="",
    ...
    def handle(self, *args, **options):
        ...
        for pasta in subpastas:
            ...
            nome_produto = titulo_en or titulo_pt or pasta.name
            ...
            if not fotos:
                ...
                continue
            foto_path = fotos[0]  # primeira foto (sorted = ordem alfabética)
            try:
            ...
            # --- pipeline 3D (opcional) ---
            glb: bytes | None = None
            dimensions: str = ""

            tmf_path = pasta / "modelo.3mf"
            tem_3mf = tmf_path.exists() and not sem3d
            ...
            if tem_3mf:
                try:
                    ...
                    # dimensões ANTES de finalizar (finalizar_glb rotaciona a malha)
                ...
                if not sem3d and not tmf_path.exists():
